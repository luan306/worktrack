...
export default function CompletedPage() {
  ...
  const fmtDate = d => d ? new Date(d).toLocaleString(undefined, { day:'2-digit', month:'2-digit', year:'numeric', hour:'2-digit', minute:'2-digit' }) : '—';

  return (
    <div className="flex-1 flex flex-col overflow-hidden">
      {/* Topbar */}
      <div className="px-6 py-3 bg-white border-b flex items-center gap-3 flex-shrink-0 flex-wrap">
        <h1 className="font-bold text-[#1e2a3a] flex-1">✅ {t('completed')}</h1>
        <input type="date" className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm outline-none" value={dateFrom} onChange={e=>setDateFrom(e.target.value)} />
        <span className="text-xs text-gray-400">to</span>
        <input type="date" className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm outline-none" value={dateTo} onChange={e=>setDateTo(e.target.value)} />
        <input className="border border-gray-200 rounded-lg px-3 py-1.5 text-sm outline-none focus:border-[#3a7bd5] w-44"
          placeholder="🔍 Search..." value={search} onChange={e=>setSearch(e.target.value)} />
      </div>

      {/* Summary */}
      <div className="px-6 py-3 bg-gray-50 border-b flex gap-3 flex-shrink-0 flex-wrap">
        {[
          { label:'Total', val: tasks.length, color:'text-[#1e2a3a]', bg:'bg-white' },
          { label:'✅ On time', val: ontime, color:'text-green-700', bg:'bg-green-50 border-green-200' },
          { label:'⚠️ Late',    val: late,   color:'text-red-600',   bg:'bg-red-50 border-red-200' },
        ].map(s => (
          <div key={s.label} className={`${s.bg} border rounded-xl px-5 py-3 flex items-center gap-3`}>
      ...
      <div className="flex border-b bg-white px-6 flex-shrink-0">
        {[['all','All'],['ontime','✅ On time'],['late','⚠️ Late']].map(([k,l]) => (
      ...
      <div className="flex-1 overflow-y-auto p-5 flex flex-col gap-3">
        ...
        {!loading && filter !== 'late' && tasks.filter(t=>!t.is_late).length > 0 && (
          <>
            <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-xl border border-gray-200">
              ...
              <span className="font-bold text-green-700 flex-1">On time</span>
              <span className="text-xs font-bold bg-green-100 text-green-700 px-2 py-0.5 rounded-full">{tasks.filter(t=>!t.is_late).length} tasks</span>
            </div>
        ...
        {!loading && filter !== 'ontime' && tasks.filter(t=>t.is_late).length > 0 && (
          <>
            <div className="flex items-center gap-2 px-4 py-2 bg-white rounded-xl border border-red-200 bg-red-50/50">
              ...
              <span className="font-bold text-red-600 flex-1">Late</span>
              <span className="text-xs font-bold bg-red-100 text-red-600 px-2 py-0.5 rounded-full">{tasks.filter(t=>t.is_late).length} tasks</span>
            </div>
        ...
        {!loading && !tasks.length && (
          <div className="text-center py-12 text-gray-400 text-sm">No completed tasks found</div>
        )}
      </div>
    </div>
  );
...
function TaskCard({ task, fmtDate, late=false }) {
  return (
    <div className={`card p-4 border-l-4 ${late ? 'border-l-red-400 bg-red-50/20' : 'border-l-green-400'}`}>
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap">
            ...
            {late ? <span className="badge badge-red">⚠️ Late</span> : <span className="badge badge-green">✅ On time</span>}
            {task.score != null && <span className="badge badge-blue">⭐ {task.score} pts</span>}
          </div>
